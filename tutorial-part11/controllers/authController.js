...
const handleLogin = async (req, res) => {
  ...
  if (match) {
    ...
    const accessToken = jwt.sign(
      ...
      { expiresIn: "30s" } //access tokens should expire relatively quickly (15 min or so), but is set to 30 sec here for tutorial purposes
    );
    const refreshToken = jwt.sign(
      { username: foundUser.username },
      process.env.REFRESH_TOKEN_SECRET,
      { expiresIn: "1d" } // refresh tokesn should last longer, hours to days depending on preferences, but should still expire at some point
    );
